  ...
  create(context) {
    ...
    return {
      ...
      JSXExpressionContainer(node) {
        ...
        const isNotInAttribute = parentType !== AST_NODE_TYPES.JSXAttribute;
        const isNotInElement = parentType !== AST_NODE_TYPES.JSXElement;
        const isUnsupportedAttribute =
          parentType === AST_NODE_TYPES.JSXAttribute && !propsToCheck.includes(String(parent.name.name));

        if (isUnsupportedAttribute || (isNotInAttribute && isNotInElement)) {
          return;
        }
        ...
        if (untranslatedExpressions.length && !hasTransAncestor) {
          untranslatedExpressions.forEach((nodeToReport) => {
            const errorCanBeFixed = canBeFixed(nodeToReport, context);
            context.report({
              node: nodeToReport,
              messageId,
              suggest: errorCanBeFixed
                ? [
                    {
                      messageId: 'wrapWithT',
                      fix: getTFixers(nodeToReport, context),
                    },
                  ]
                : undefined,
            });
      ...
      'JSXElement, JSXFragment'(node) {
        ...
        const parentHasText = parentHasChildren
          ? parent.children.some((child) => {
              ...
              const shouldBeTranslated = stringShouldBeTranslated(childValue);
              return child.type === AST_NODE_TYPES.JSXText && shouldBeTranslated;
            })
          : false;
